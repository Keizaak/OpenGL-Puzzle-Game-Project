...
PA3Application::PA3Application(int windowWidth, int windowHeight)
        : Application(windowWidth, windowHeight, "Application for PA3"),
          m_program(new Program("shaders/simple3d.v.glsl", "shaders/simple3d.f.glsl")), m_view(1), m_currentTime(0),
          m_deltaTime(0),
...
std::shared_ptr<VAO> PA3Application::makeParamSurf(DiscreteLinRange rgPhi, DiscreteLinRange rgTheta,
                                                   const std::function<glm::vec3(float, float)> &posFunc,
                                                   bool isCyclicInPhi, bool isCyclicInTheta) {
    std::vector<glm::vec3> positions;
    std::vector<glm::vec3> colors;
    std::vector<uint> ibo;
    auto index = [&](unsigned int kphi, unsigned int ktheta) {
        return kphi % rgPhi.nbVals * rgTheta.nbVals + ktheta % rgTheta.nbVals;
    };

    for (unsigned int kphi = 0; kphi < rgPhi.nbVals; kphi++) {
        for (unsigned int ktheta = 0; ktheta < rgTheta.nbVals; ktheta++) {
            ...
            glm::vec3 blue = {0, 0, 1};
            glm::vec3 red = {1, 0, 0};
            float t = kphi / static_cast<float>(rgPhi.nbVals);
            colors.push_back(t * red + (1 - t) * blue);

            if ((kphi <= rgPhi.nbVals - 2 || isCyclicInPhi) && (ktheta <= rgTheta.nbVals - 2 || isCyclicInTheta)) {
                ibo.push_back(index(kphi, ktheta));
                ibo.push_back(index(kphi + 1, ktheta));
                ibo.push_back(index(kphi, ktheta + 1));

                ibo.push_back(index(kphi, ktheta + 1));
                ibo.push_back(index(kphi + 1, ktheta));
                ibo.push_back(index(kphi + 1, ktheta + 1));
            }
        }
    }
...
void PA3Application::makeASphere(unsigned int nbPhi, unsigned int nbTheta) {
    auto posFunc = [](float phi, float theta) {
        return glm::vec3(cosf(phi) * sinf(theta), sinf(phi) * sinf(theta), cosf(theta));
    };

    const float pi = glm::pi<float>();
    std::shared_ptr<VAO> vao = makeParamSurf(DiscreteLinRange(nbPhi, 0, 2 * pi), DiscreteLinRange(nbTheta, 0, pi),
                                             posFunc, true, false);
    glm::mat4 mw(1);
...
void PA3Application::makeATorus(unsigned int nbPhi, unsigned int nbTheta, float smallRadius) {
    ...
    auto posFunc = [&](float phi, float theta) {
        ...
        return glm::vec3(cosf(phi) * (1 + smallRadius * cosf(theta)), smallRadius * sinf(theta),
                sinf(phi) *(1 + smallRadius * cosf(theta)));
    };

    auto pi = glm::pi<float>();
    vao = makeParamSurf(DiscreteLinRange(nbPhi, 0, 2 * pi), DiscreteLinRange(nbTheta, 0, 2 * pi), posFunc, true, false);
...
void PA3Application::makeAShell(unsigned int nbPhi, unsigned int nbTheta) {
    auto posFunc = [&](float theta, float phi) {
        return glm::vec3(-phi * sinf(theta) * sinf(theta) * sinf(phi), phi * sinf(theta) * sinf(theta) * cosf(phi),
                         phi * sinf(theta) * cosf(theta));
    };

    auto pi = glm::pi<float>();
    std::shared_ptr<VAO> vao = makeParamSurf(DiscreteLinRange(nbPhi, 0, pi),
                                             DiscreteLinRange(nbTheta, -pi / 4, 5 * pi / 2), posFunc, false, false);
    glm::mat4 mw(1);
    mw = glm::translate(mw, {-0.5, 0, 0});
    mw = glm::rotate(mw, 3 * glm::pi<float>() / 4, {1, 0, 1});
...
PA3Application::RenderObject::RenderObject(const std::shared_ptr<Program> &prog, const std::shared_ptr<VAO> &vao,
                                           const glm::mat4 &modelWorld) : m_prog(prog), m_vao(vao), m_mw(modelWorld) {}

std::shared_ptr<PA3Application::RenderObject>
PA3Application::RenderObject::createInstance(const std::shared_ptr<Program> &prog, const std::shared_ptr<VAO> &vao,
